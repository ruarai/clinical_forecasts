...
t_parameters <- list(
  tar_target(date_forecasting, ymd("2023-07-28")),
  
  
  tar_target(date_simulation_start, date_forecasting - days(28 * 6)),
  tar_target(forecast_name, str_c("fc_", date_forecasting, "_test_pf")),
  
  # Update these to the latest file path
  # ~/mfluxshared and ~/mfluxunimelb should point to the (respective) mediaflux server
  tar_target(raw_nindss, "~/mfluxshared/Health Uploads/COVID-19 UoM 6months-27July2023.zip"),
  tar_target(raw_local_cases, "~/mfluxunimelb/local_cases_input/local_cases_input_2023-07-27.csv"),
  ...
  tar_target(raw_ensemble, "~/mfluxshared/forecast-outputs/combined_samples_varasc2023-07-21.csv"),
  tar_target(occupancy_path, "data/occupancy/NAT_2023-07-27_Data for Uni of Melbourne.xlsx"),

...
nindss_date <- tar_read(forecast_dates)$NNDSS
simulation_start_date <- tar_read(forecast_dates)$simulation_start
...
age_tables <- map(
  ...
  function(i_bootstrap) {
    nindss_state %>%
      ...
      ungroup() %>%
      arrange(age_group, desc(date_onset)) %>%
...
plot_data <- all_results %>%
  pivot_longer(cols = c(pr_age_given_case, pr_hosp, pr_ICU))


ggplot(plot_data) +
  geom_line(aes(x = window_start + window_width, y = value, group = bootstrap),
            alpha = 0.1) +
  
  xlab(NULL) + ylab(NULL) +
  
  facet_wrap(~age_group * name, scales = "free_y", ncol = 3, labeller = label_wrap_gen(multi_line=FALSE)) +

...
produce_input_trajectories_NSW <- function(simulation_options,
  ...
  na_forecast_vals <- any(is.na(ensembles_wide %>% select(-c(state, date))))
  
  if(na_forecast_vals) {
    print("NA values in forecast trajectories, imputing")
    
    ensembles_wide <- ensembles_wide %>%
      fill(-c(state, date), .direction = 'downup')
  }
